...
uint8_t area_size[tcs_system + 1] __RETAINED;
...
#else //EXT_CRYSTAL_IS_32M
        (uint32_t)&CRG_TOP->CLK_FREQ_TRIM_REG, 0x0680,
#endif
...
void sys_tcs_sort_array(void)
{
...
#else //EXT_CRYSTAL_IS_32M
                CRG_TOP->CLK_FREQ_TRIM_REG = 0x0680;
#endif
        }
        ...
        if (sys_tcs_is_calibrated_chip) {
                ...
                area_size[tcs_system] = tcs_length - entry_ptr;
...
void sys_tcs_apply(sys_tcs_area_t area)
{
        ...
        for (i = area_offset[area]; i < (area_offset[area] + area_size[area]); i += 2) {
                apply_entry(i);
        }
}
