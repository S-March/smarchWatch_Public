...
static bool hw_rf_iff_calibration(iff_mode_ctrl_e mode_ctrl)
{
        ...
        // Save the values of registers that will be changed in the function
        uint16_t rf_cal_ctrl_reg_value = RFCU->RF_CAL_CTRL_REG;
        uint16_t rf_overrule_reg_value = RFCU->RF_OVERRULE_REG;
...
#else
        iff_result = hw_rf_iff_calibration(0x0); // Dummy Call - Keep GCC happy
        iff_result = false;
#endif
#endif
        if (iff_result) {
