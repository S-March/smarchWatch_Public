...
__STATIC_INLINE void *ke_malloc(uint32_t size)
{
    return kmalloc(size, GFP_KERNEL);
}

__STATIC_INLINE void ke_free (void * mem_ptr)
{
    kfree(mem_ptr);
}
...
__STATIC_INLINE void *ke_malloc(uint32_t size) { return malloc(size); }

__STATIC_INLINE void ke_free(void * mem_ptr) { free(mem_ptr); }
